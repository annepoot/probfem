...
def save_mesh(elems, fname):
    file, extension = os.path.splitext(fname)

    if extension == ".mesh":
        _write_mesh(elems, fname)
    elif extension == ".msh":
        _write_msh(elems, fname)
    else:
        raise ValueError("Invalid file type passed to save_mesh")


def _write_mesh(elems, fname):
...
def _write_msh(elems, fname):
    ...
    with open(fname, "w") as file:
        ...
        for ielem, inodes in enumerate(elems):
            ...
            elem_type = get_msh_elem_type(rank, len(inodes))
...
def get_msh_elem_info(elem_type):
    if elem_type < 1 or elem_type > 19:
        raise ValueError("invalide elem_type")
    rank, nodecount = msh_elem_list[elem_type]
    return rank, nodecount


def get_msh_elem_type(rank, node_count):
    if (rank, node_count) not in msh_elem_list:
        raise ValueError("invalide rank-nodecount combination")
    else:
        return msh_elem_list.index((rank, node_count))
