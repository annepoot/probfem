...
for h in [0.2, 0.1, 0.05]:
    for h_meas in [0.2, 0.5, 1.0]:
        for std_corruption in [1e-3, 1e-4, 1e-5]:
            subdf = df[abs(df["h"] - h) < 1e-8]
            subdf = subdf[abs(subdf["h_meas"] - h_meas) < 1e-8]
            subdf = subdf[abs(subdf["std_corruption"] - std_corruption) < 1e-8]
            ...
            ax.set_aspect("equal")
            plt.axis("off")
            plt.title(f"$h = {h}$, $h_m = {h_meas}$, $\sigma_e = {std_corruption:.0e}$")
